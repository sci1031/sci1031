...
Dans la section Leçon, vous utiliserez des données d'élévation pour le Québec et les limites de quatre parcs nationaux québécois.
...
Pour obtenir le sentier estival qui va au plus proche du sommet, nous allons avoir besoin de croiser deux couches d'informations spatiales: (i) [les sentiers estivaux de la SEPAQ](https://www.donneesquebec.ca/recherche/dataset/sentiers-estivaux) mentionnés plu haut et (ii) la coordonnée géographique du sommet que nous avons isolé précédemment. 

Dans ce qui suit, en vue de répondre à la question posée, nous allons: 

1. importer les données des sentiers estivaux de la SEPAQ (les autres données étant déjà importées),

2. s'assurer que l'objet spatial importé et `parcs` ont le même SCR,
...
À présent, nous allons isoler les sentiers du parc du Mont-Mégantic qui se trouvent dans `sentier`. Commençons par inspecter la table des attributs&nbsp;:
...
soit un filtre spatial avec les limites du parc Mont-Mégantic que contiennent `parcs`&nbsp;:
...
Il s'agit d'une des sections du Sentier du Mont-Mégantic et nous décidons de parcourir ce sentier pendant notre randonnée, nous allons combiner les différentes portions du sentier. Commençons par sélectionner ces portions en utilisant la colonne `Toponyme1`.
...
```
Nous allons combinant dans l'ordre du sentier depuis, avec `st_combine()`, en débutant avec la section 5: 
...
1. Nous avons de convertir `rando` qui est de classe `sfc` en objet de classe `sf` pour l'utiliser avec `extract()`, ce que nous faisons avec `st_as_sf()`,

2. `along = TRUE` nous permet d'avoir les cellules ordeonnées le long des lignes de `rando_sections`,

3. `cellnumbers = TRUE` nous permet d'avoir les numéros de cellules extraites.


Comme vu un peu plus haut, `topo_elv` est une liste, mais cette fois le premier élément est une matrice avec deux colonnes: une pour l'identifiant des cellules, la seconde colonne à la valeur d’élévation (précédemment, c'était un vecteur avec seulement des valeurs d'élévation).  
...
Il nous reste alors à utiliser la distance `st_distance()` pour calculer les distance entre points successifs. Une astuce pour le faire est d'utiliser deux fois `topo_pts`, une fois sans le premier point et la deuxième fois sans le dernier élément et d'utiliser une distance élément par élément avec l'argument `by_element = TRUE`, ce qui nous permet de faire la distance entre points qui se suivent.
...
Pour répondre à notre question. il nous reste à visualiser l'élévation (la deuxième colone de `topo_elv[[1]][, 2]`) en fonction de la somme cumulée des distances que nous venons de calculer (avec `cumsum()`). Remarquons que le calcul précédent nous donne une valeur de distance de moins (car nous calculons des intervalles), nous rajoutons donc le 0 initial aux distances calculées.
