...
Afin de faciliter le téléchargement de ces données, l'ensemble de ces couches d'informations spatiales peuvent être téléchargée en cliquant sur un seul lien: [données pour le module 8](https://github.com/sci1031/data/raw/main/Module8_donnees.zip). Une fois téléchargé, le dossier compressé (`zip`) doit être dézippé dans votre répertoire de travail. Le dossier `Module8_donnees` comprend deux sous-dossiers et un fichier:
...
Dans allons d'abord importer les différentes couches d'informations spatiales dans l'environnement `R`. Nous commençons par charger les bibliothèques requises pour importer les données spatiales vectorielles (`sf`), pour importer les données spatiales matricielles (`terra`) et pour visualiser ces données (`mapview`).

```{r load80, warning = FALSE, results = "hide", message = FALSE}
library(sf)
library(terra)
library(mapview)
...
Enfin, nous importons la couche d'élévation `DEM` pour la région d'intérêt en utilisant la fonction `rast()` de la bibliothèque `terra`.

```{r, warning = FALSE}
dem <- rast("Module8/Module8_donnees/DEM.tif")
```
<br>
...
mapview(dem) + mapview(parcs, zcol = "TRQ_NM_", label = "TRQ_NM_")
```
<br>

Notez que l'attribut `TRQ_NM_` correspond aux noms des parcs nationaux. Notez également que l'argument `label` permet d'afficher la valeur de l'attribut associé (ici le nom des parcs) lorsque la souris survole le polygone de cette carte interactive.
...
Au [module 5](#mat), nous avons vu comment accéder et manipuler les données de *raster* en utilisant, entre autres, les fonctions `summary()` et `values()`. Il est ainsi possible d'avoir une idée de la distribution des valeurs du *raster* `dem` en utilisant la ligne de commande suivante :
...
Plus précisément, nous allons utiliser la fonction `which()` pour filtrer les données du *raster* `dem`. Rappelons que la fonction `which()` identifie la position des éléments de valeur `TRUE` dans un vecteur logique (voir le [module 7](#fct_which) pour un rappel). Utilisons donc cette fonction pour trouver les cellules qui ont des valeurs d'élévation négatives.

```{r}
ind <- which(values(dem) < 0)
ind
```
<br>
...
L'élévation minimale est maintenant de `r min(values(dem), na.rm = TRUE)` et le nombre de `NA` a augmenté de `r length(ind)`. Ceci confirme que notre filtre a bien été appliqué.

La fonction `which()` peut aussi être utilisée pour filtrer des données selon des expressions logiques plus complexes. Nous pouvons, par exemple, remplacer `values(dem) < 0` par `values(dem) < 0 | values(dem) > 1000` pour exclure également les cellules avec des valeurs plus grandes que 1000&nbsp;m. De plus, il est aussi possible de changer les valeurs des cellules identifiées en d'autres valeurs que `NA`.

<br>

###### Fonction `classify()` {-}

La fonction `classify()` de la bibliothèque `terra` permet également de filtrer des données matricielles et de leur assigner de nouvelles valeurs.  
...
Avant de pouvoir utiliser la fonction `classify()`, il est nécessaire de construire une matrice indiquant les bornes limites des différentes classes.
...
Cette matrice est utilisée comme argument de la fonction `classify()` afin d'assigner les nouvelles classes aux valeurs du *raster* `dem`.

```{r reclassify}
nouvelles_classes_dem <- classify(dem, nouvelles_classes, right = FALSE)
```
<br>

Notez que l'utilisation par défaut de la fonction `classify()` inclue la borne supérieure mais pas la borne inférieure de l'intervalle de reclassification (`]limite_lim, limite_max]`). L'ajout de l'argument `right = FALSE` vient spécifier que nous souhaitons le contraire, c'est-à-dire l'inclusion de la borne inférieure mais pas de la borne supérieure (`[limite_lim, limite_max[`). Ainsi, nous avons précisé que les valeurs incluses dans la nouvelle classe 1 vont de 0 à 249 m plutôt que de 1 à 250 m.
...
mapview(classify(dem, nouvelles_classes2))
```
<br>
...
Il est très important de relever que de manière implicite les valeurs des coordonnées utilisées dans `ext()` sont exprimées dans le SCR de notre *raster* `dem`. 
...
En guise d'exemple, nous allons maintenant rogner le *raster* `dem` en utilisant les limites du premier parc de la SÉPAQ identifié dans les données vectorielles `parcs`. Il est en effet possible d'extraire un objet de classe `Extent` à partir de ces données. Vérifions-le en utilisant la fonction `ext()`:
...
```{r mask, warning = FALSE, message = FALSE, dev = "png"}
dem_ma <- mask(dem, pol)
mapview(dem_ma)
```
<br>
...
```{r mask_inv, warning = FALSE, message = FALSE, dev = "png"}
...
>
>iii. Dans la situation où seules les valeurs des cellules filtrées nous intéresse, il est possible d'utiliser la fonction `extract()` plutôt que la fonction `mask()`. La fonction `extract()` de la bibliothèque `terra` retourne un *data.frame* dont la colonne `ID` identifie le polygone d'origine (ici toujours `1` puisqu'il n'y a qu'un seul polygone) et dont la deuxième colonne contient les valeurs des cellules extraites.
>

Le *data.frame* retourné possède `r nrow(extract(dem, vect(pol)))` lignes, chacune correspondant à une cellule extraite.

```{r, echo = FALSE, eval = TRUE, warning = FALSE}
dem_ex <- extract(dem, vect(pol))
...
Il est également possible d'agréger les valeurs extraites directement lors de l'extraction en passant une **fonction** à l'argument `fun`. Par exemple, pour calculer l'**élévation moyenne** à l'intérieur de chacun des quatre parcs nationaux, nous pouvons écrire&nbsp;:

```{r extract_fun}
dem_parcs_moy <- extract(dem_lcc, vect(parcs), fun = mean, na.rm = TRUE)
dem_parcs_moy
```
<br>

Le résultat est un *data.frame* dont la colonne `ID` identifie chaque polygone (de 1 à 4, correspondant aux quatre parcs) et dont la deuxième colonne contient la valeur agrégée — ici la moyenne d'élévation. Il est alors possible d'ajouter ces valeurs directement à l'objet vectoriel `parcs`&nbsp;:

```{r extract_ajout}
parcs$elev_moy <- dem_parcs_moy[, 2]
parcs[, c("TRQ_NM_", "elev_moy")]
```
<br>

Cette approche est particulièrement utile lorsqu'on souhaite caractériser des unités spatiales (polygones) à partir d'une couche matricielle, sans avoir à passer par `mask()` pour chaque polygone individuellement.
<br>
...
```{r merge, echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'left', out.width = '100%', dev = "png"}
library(leaflet)
library(leafsync)
...
```{r mergeordre, echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'left', out.width = '100%', dev = "png"}
...
```{r mosaic, echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'left', out.width = '100%', dev = "png"}
...
Nous pouvons maintenant trouver la valeur d'élévation la plus élevée en utilisant la fonction `values()` suivie de la fonction `max()`. 

```{r}
vmax <- max(values(dem_parcs), na.rm = TRUE)
vmax
```

<br>
Nous avons ainsi obtenu l’élévation maximale de ces parcs qui est de `r format(vmax, digits = 4)`&nbsp;m. Nous cherchons alors les coordonnées spatiales associées à cette valeur. Nous pouvons faire appel à la fonction `which()` pour trouver l'**indice** de la cellule (ou les indices des cellules, s'il y en a plusieurs) en question.

```{r}
ind_max <- which(values(dem_parcs) ==  max(values(dem_parcs), na.rm = TRUE))
ind_max 
```
...
Pour déterminer les coordonnées de la cellule identifiée, nous utilisons la fonction `xyFromCell()` qui retourne les coordonnées spatiales associées à l'indice d'une cellule dans un *raster*. Pour illustrer son fonctionnement, appliquons-la d'abord aux 5 premières cellules de `dem_parcs`&nbsp;:

```{r}
# Exemple : coordonnées des 5 premières cellules
xyFromCell(dem_parcs, 1:5)
```
<br>

Appliquons maintenant `xyFromCell()` à la cellule d'élévation maximale&nbsp;:

```{r}
# Coordonnées de la cellule d'élévation maximale
df_max <- as.data.frame(xyFromCell(dem_parcs, ind_max))
df_max
```
...
```{r sommet, fig.cap = "En cliquant sur le polygone du parc contenant le point rouge (point d'élévation maximal), il est possible de constater que le point se retrouve dans le Parc national du Mont-Mégantic", warning = FALSE, message = FALSE, dev = "png"}
...
Maintenant importons les données vectorielles pour tous les sentiers de la [SÉPAQ](https://www.donneesquebec.ca/recherche/dataset/sentiers-estivaux). Ce *shapefile* se trouve dans le dossier `Module8_donnees` que vous avez téléchargé au début de la leçon&nbsp;:
...
library(elevatr)
elv_megantic <- get_elev_raster(parc_megantic, z = 14) 
...
elv_megantic <- rast("Module8/Module8_donnees/megantic_elevation.grd")
```
<br>
...
```{r sentier_megantic, warning = FALSE, message = FALSE, dev = "png"}
...
Pour faire le profil topographique du sentier, nous devons extraire les données d’élévation du raster `elv_megantic` le long du sentier. Pour que le profil soit correct, il est essentiel que les valeurs d’élévation soient extraites dans l’**ordre de traversée** du sentier, c’est-à-dire du début à la fin du trajet. Or, la fonction `extract()` appliquée directement à une géométrie de ligne ne garantit pas cet ordre.

Pour assurer l’ordre de traversée, nous commençons par convertir le sentier `rando` en une série de **points** à l’aide de `st_cast()`. Cette fonction décompose chaque section (géométrie de type ligne) en ses **vertex** — les points qui définissent la géométrie — en les retournant dans leur ordre d’apparition le long de la ligne. Contrairement à une extraction le long d’une ligne, l’extraction à des points garantit que `terra` retourne les valeurs dans l’ordre des points fournis.

```{r elv_profile}
rando_pts <- st_cast(rando, "POINT")
topo_elv <- extract(elv_megantic, vect(rando_pts), cells = TRUE)
```
<br>
...
>
>i. `st_cast(rando, "POINT")` décompose le MULTILINESTRING `rando` en ses vertex successifs, retournés dans leur ordre d’apparition le long de chaque section composante. L’ordre de traversée du sentier de bout en bout est ainsi préservé dans `rando_pts`.
>

<br>

>
>ii. Nous avons besoin de convertir `rando_pts` qui est de classe `sf` en objet de classe `SpatVector` pour l’utiliser avec `extract()`, ce que nous faisons avec `vect()`.
>

<br>

>
>iii. `cells = TRUE` nous permet d’avoir les indices des cellules extraites.
>

<br>

Comme vu un peu plus haut, la fonction `extract()` de `terra` retourne un *data.frame*. Ainsi, `topo_elv` est un objet de classe `data.frame` avec trois colonnes: `ID` (l’identifiant de chaque point, de 1 au nombre total de points), les valeurs d’élévation, et `cell` (les indices des cellules).

Nous pouvons confirmer ces propos par les commandes suivantes&nbsp;:
```{r}
class(topo_elv)   # topo_elv est un objet de classe "data.frame"
nrow(topo_elv)    # nombre de cellules extraites
names(topo_elv)   # noms des colonnes
```
<br>

Renommons les colonnes pour plus de clarté&nbsp;:
```{r}
names(topo_elv) <- c("point_id", "elevation", "cellule_id")
head(topo_elv)
```
...
Tout d'abord, nous devons calculer la distance parcourue entre chaque paire de points qui se suivent le long du sentier. Pour ce faire, nous utiliserons la fonction [`st_distance()`](#fct_distance) vue au module 7. Or, cette fonction ne peut être utilisée que sur un objet spatial, et comme nous venons de le constater `topo_elv` est un objet de classe `data.frame`. Nous devons donc transformer `topo_elv` en objet spatial avec la fonction `st_as_sf()`. 

De plus, un objet spatial doit être défini par des coordonnées spatiales. Pour l'instant, les cellules formant le sentier dans `topo_elv` sont identifiées par leur indice et non par leurs coordonnées. Rappelons la fonction `xyFromCell()` vue plus haut pour retrouver les coordonnées spatiales correspondant à chaque indice dans `elv_megantic`. 

```{r xyFromCell}
df_pts <- as.data.frame(xyFromCell(elv_megantic, topo_elv$cellule_id))
head(df_pts)
```
<br>

Nous avons mis les coordonnées spatiales dans le tableau `df_pts` de type `data.frame`. Notez que `topo_elv$cellule_id` correspond aux indices des cellules extraites. 
...
```{r}
dist_pts <- st_distance(topo_pts[-1, ], topo_pts[-nrow(topo_pts),], by_element = TRUE)
```
<br>
...
Nous pouvons finalement visualiser l'élévation (la colonne `topo_elv$elevation`) en fonction de la somme cumulée des distances, `dist_parcourue`. Remarquons que le vecteur `dist_parcourue` comprend un élément de moins que la colonne `topo_elv$elevation`. Ceci s'explique par le fait que `dist_pts`, à partir duquel nous calculons `dist_parcourue`, mesure la distance entre les points (c'est-à-dire des intervalles). Par conséquent, nous ajoutons la distance initiale de 0&nbsp;m au vecteur des distances parcourues.

```{r profile_topo, fig.width = 9.5, dev = "png"}
plot(c(0, dist_parcourue), topo_elv$elevation,
...
```{r, warning=FALSE, eval=FALSE, message = FALSE, dev = "png"}
dem_orford <- mask(dem_lcc, parc_orford)
mapview(dem_orford)
```
```{r, warning=FALSE, echo=FALSE, message = FALSE, dev = "png"}
dem_orford <- mask(dem_lcc, parc_orford)
...
Déterminer l'indice de la cellule ou des cellules d'élévation maximale en utilisant les fonctions `values()` et `which.max()`.

```{r}
imax_orford<-which.max(values(dem_orford))
```
...
```{r, warning=FALSE}
quantile_orford <- quantile(values(dem_orford), na.rm = TRUE)
quantile_orford
...
Utiliser la fonction `classify()` pour assigner les nouvelles classes au *raster* `dem_orford`.

```{r}
dem_orford_quantile <- classify(dem_orford, classes_orford, right = FALSE)
...
imax_megantic <- which.max(values(dem_megantic))
...
imax_yamaska <- which.max(values(dem_yamaska))
...
imax_frontenac <- which.max(values(dem_frontenac))
...
```
```{r, warning=FALSE, echol=FALSE, message = FALSE, dev = "png"}
...
```{r, warning=FALSE, eval=FALSE, warning = FALSE, message = FALSE, dev = "png"}
mapview(dem_tampon_max)
```

```{r, warning=FALSE, echo=FALSE, warning = FALSE, message = FALSE, dev = "png"}
