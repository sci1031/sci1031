...
#### Calculer la superficie des cellules {-}

Une autre information pertinente à connaître à propos d'un *raster* est la superficie de ses cellules. La fonction `cellSize()` de la bibliothèque `terra` retourne un nouveau *raster*, de mêmes dimensions que le *raster* original, dont la valeur de chaque pixel correspond à la superficie (en m²) de la cellule correspondante.
...
Nous remarquons que la superficie des cellules varie légèrement autour de 1605 m², alors que la résolution du *raster* `T_vdq` est de 40 m par 40 m (donc une superficie nominale de 1600 m²). Cette légère différence provient de la déformation inhérente à toute projection cartographique.

Il est aussi possible d'exprimer la superficie des cellules dans une autre unité de mesure, comme le kilomètre carré, en précisant l'argument `unit` de la fonction `cellSize()`.

```{r}
cellSize(T_vdq, unit = "km")
```
...
Vérifions l'effet de l'agrégation sur la superficie des cellules. Puisque nous avons augmenté la résolution par un facteur `fact = 4` (une cellule 4 fois plus large et 4 fois plus haute), nous nous attendons à ce que la superficie des cellules soit 16 fois plus grande.

```{r}
cellSize(T_sec)
cellSize(T_fact4_moy)
```
<br>
Effectivement, `r round(global(cellSize(T_sec), "min")$min, 0)` × 4² = `r format(round(global(cellSize(T_sec), "min")$min * 16, 0), scientific = FALSE)`, ce qui correspond bien à la superficie obtenue pour `T_fact4_moy` (`r format(round(global(cellSize(T_fact4_moy), "min")$min, 0), scientific = FALSE)`).
...
Comparons maintenant la superficie des cellules avant et après la transformation de projection.
...
Nous remarquons que même si la résolution du *raster* `T_mtm7` n'est plus exactement 40 m par 40 m, la superficie des cellules demeure très similaire à celle du *raster* `T_vdq` d'origine.
...
**b)** Trouver l'étendue, les dimensions, la résolution, et la superficie des cellules (valeurs minimale et maximale) du *raster* associé à l'année 2001.
...
dim(C2001)
res(C2001)
global(cellSize(C2001), c("min", "max"))
```
