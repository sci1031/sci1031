...
```{r,  warning = FALSE, fig.dim = c(5, 5), fig.cap="Visualisation du raster `M` avec la fonction `mapview()`"}
library(mapview)
mapview(M)
```
<br>
...
```{r, warning = FALSE,  fig.dim = c(5, 5), fig.cap="Raster dont les valeurs sont de type logique"}
mapview(M_logique)
```
<br>
...
```{r, warning = FALSE,   fig.dim = c(5, 5), fig.cap="Raster dont les valeurs sont de type facteur"}
mapview(M_factor)
```
<br>
...
```{r, warning = FALSE, fig.dim = c(7, 7), fig.cap="Carte de la température de surface dans la ville de Québec (visualisation de tous les pixels)"  }
