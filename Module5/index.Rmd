...
T_merc <-project(T_vdq, "EPSG:3857")
```
<br>
...
#T_crop <- crop(T_vdq, ext(xFromCol(T_vdq, 351), xFromCol(T_vdq, 450),
  #                         yFromRow(T_vdq, 750), #yFromRow(T_vdq, 651)))

ext_sec <- ext(xFromCol(T_vdq, 351), xFromCol(T_vdq, 450),
               yFromRow(T_vdq, 450), yFromRow(T_vdq, 351))
T_crop <- crop(T_vdq, ext_sec)

T_crop_mtm7 <- project(T_crop, proj_mtm7)
T_crop_merc <-  project(T_crop, "EPSG:3857")
par(mfrow = c(1,3))
plot(T_crop, main = "Québec Lambert")
plot(T_crop_mtm7, main = "MTM fuseau 7")
plot(T_crop_merc, main = "Mercator web")
...
Pour sélectionner une région précise dans un *raster*, nous avons utilisé la fonction `crop()`. Nous reviendrons en détail sur cette fonction dans le [module 8](#manip_mat) qui traite de la manipulation de données matricielles.
