...
```{r, fig.dim = c(5, 5), fig.align='center', fig.cap="Visualisation du raster `M` avec la fonction `plot()`" }
plot(M)
```
<br>
...
```{r, warning = FALSE, echo = FALSE, fig.dim = c(5, 5), fig.cap="Raster dont les valeurs sont de type facteur"}
m = mapview(M_factor)
m@map
```
...
```{r, fig.dim = c(5, 5), fig.align='center', fig.cap="Histogramme de la distribution des valeurs dans le raster G"}
hist(G,
     main = "",
     xlab = "Valeurs",
     ylab = "Fréquence",
...
```{r, fig.dim = c(4, 4), fig.align='center', fig.cap="Valeur moyenne et intervalle de confiance du raster G"}
boxplot(G,
        main = "",
        ylab = "Valeurs",
        col = "darkorange")
...
```{r, fig.dim = c(6, 5), fig.align='center', fig.cap = "Distribution des températures de surface dans la ville de Québec"}
...
```{r, fig.dim = c(8, 8), fig.cap = "Cartes de différentes résolutions", eval = TRUE}
...
```{r latticeView, collapse = TRUE, warning = FALSE, fig.height=10, fig.width = 10,fig.show='hold',fig.align='center', fig.cap="Carte pour chacune des bandes rouge, verte et bleue"}
...
```{r viewRG, fig.cap = "Carte combinant les trois bandes de couleurs", warning = FALSE, message = FALSE}
