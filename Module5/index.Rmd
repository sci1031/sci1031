...
```{r,  warning = FALSE, message = FALSE, fig.dim = c(5, 5), fig.cap="Visualisation du raster `M` avec la fonction `mapview()`"}
library(mapview)
mapview(M)
```
<br>
...
```{r, warning = FALSE, message = FALSE, fig.dim = c(5, 5), fig.cap="Raster dont les valeurs sont de type logique"}
mapview(M_logique)
```
<br>
...
```{r, warning = FALSE, message = FALSE, fig.dim = c(5, 5), fig.cap="Raster dont les valeurs sont de type facteur"}
mapview(M_factor)
```
<br>
...
<br>
```{r, echo=FALSE,fig.cap="Structure d'un raster: indice, position et valeur des pixels", out.width = '80%'}
knitr::include_graphics('Module5/images/ValeurRaster.png')
```
<br>
...
```{r, warning = FALSE, message = FALSE, fig.dim = c(7, 7), fig.cap="Carte de la température de surface dans la ville de Québec" }
mapview(T_vdq, 
...
```{r, warning = FALSE, message = FALSE, fig.dim = c(7, 7), fig.cap="Carte de la température de surface dans la ville de Québec (visualisation de tous les pixels)"  }
...
```{r, fig.dim = c(6, 6), fig.cap="Carte de la température de surface dans le secteur de Sainte-Foy"}
...
```{r, eval=TRUE, ECHO = FALSE, fig.cap="Section des rasters de projections Québec Lambert et MTM"}
...
Finalement, pour sauvegarder des données matricielles, nous utilons la fonction `writeRaster()` de la librarie `raster`. Par exemple, sauvons les données matricielles `T_vdq` que nous avons transformées dans le système de projection MTM.
...
```{r, collapse=TRUE, warning = FALSE, message = FALSE, fig.cap="Carte pour chacune des bandes rouge, verte et bleue"}
