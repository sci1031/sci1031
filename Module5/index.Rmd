...
**c)** En utilisant la fonction `st_crs()` de la library `sf`, déterminer le code EPSG du système de coordonnées utilisé ainsi que l'unité de mesure de la projection du *raster* de la couverture terrestre de 2001.
...
* `code`: correspond à l'identifiant de chaque type de couvert;
...
sommaire <- legende[legende$code %in% type2001, ]
sommaire
```
<br>
...
Nous ajoutons les autres attributs au *data.frame* `rat`
```{r}
rat$classe <- sommaire$class
rat$description <- sommaire$description
...
sommaire <- legende[legende$code %in% type2016, ]
rat <- levels(R2016f)[[1]]
rat$classe <- sommaire$class
rat$description <- sommaire$description
...
* Utiliser l'argument `att = ` afin que la légende affiche la description des types de couvertures.
...
```{r}
mapview(R2001f, col.regions = sommaire$color, att = "description", legend = TRUE)
```
...
Nous divisons ensuite par le nombre de cellules totales dans l'un ou l'autre des rasters (ils on le même nombre), ce qui permet de comparer les catégories entre elles.
...
```{r}
sommaire$description[which.max(sommaire$Diff2016_2001)]
sommaire$description[which.min(sommaire$Diff2016_2001)]
```
<br>

Ce sont donc les champs cultivés qui ont le plus augmenté et les peuplements d'arbustres qui ont le plus diminué au cours de cette période.
