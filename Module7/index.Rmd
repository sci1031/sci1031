...
La fonction `st_combine()` de la bibliothéque `st` sert à combiner des géométries afin d'en former une seule. Cette opération peut être utile lorsque nous souhaitons considérer plusieurs géométries comme formant un même objet spatial.
...
GIM_combine
```
Le nouvel objet `GIM_combine` est effectivement formée d'une seule géométrie. Notez cependant que la fonction `st_combine()` ne fusionne pas les frontières à l'intérieur du nouveau polygone formé (\@ref(fig:map_combo)). Pour unir deux polygones, il faut plutôt utiliser la fonction `st_union()` que nous définirons plus bas dans cette leçon.
...
AB_lines_int<-st_intersection(B_lines,A_lines)
...
noms<-c("Centre_A","Centre_B","Centre_interAB", "Centre_unionAB","point_interAB")
points$position<-noms
...
map_AB <- mapview(A, col.regions = "blue", alpha.regions = 0.2, legend = FALSE) + 
  ...
  mapview(centre_A, col.regions="blue",legend = FALSE) + 
  mapview(centre_B, col.regions="red",legend = FALSE) +
  mapview(centre_interAB, col.regions="darkmagenta",legend = FALSE)+
  mapview(centre_unionAB, col.regions="darkgoldenrod",legend = FALSE)+
  mapview(point_interAB, col.regions="darkgreen",legend = FALSE)
map_AB
```
...
```{r}
st_contains(B, points)
st_contains(B, points, sparse = FALSE)
```
```{r}
points$couleur[st_contains(B, points, sparse = FALSE)]
```


```{r}
x=0
if(st_contains(B, centre_B,sparse = TRUE)==TRUE){x=1}
x
```

```{r}
st_touches(points, sym_diff_AB)
```
```{r}
st_covers(points,A)
...
mapview(a, col.regions = "blue", alpha.regions = 0.2, legend = FALSE) + 
 mapview(b, col.regions = "red", alpha.regions = 0.2, legend = FALSE) +
  mapview(c, col.regions = "green", alpha.regions = 0.2, legend = FALSE)
```
