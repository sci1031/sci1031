...
<!-- groupement se fait selon une certaine condition.-->
...
#mapview(reg_pop,zcol="Group.1")

# agg_Laurentides <-subset(agg, Group.1=="Laurentides")
# mapview(agg_Laurentides)
# 
# agg_diss<-aggregate(villes_reg_pop["population"], by = list(villes_reg_pop$Rgns_Ad), FUN = sum, na.rm = TRUE, dissolve = TRUE)
# agg_diss
# mapview(agg_diss,zcol="Group.1") 
# 
# agg_union<-aggregate(villes_reg_pop["population"], by = list(villes_reg_pop$Rgns_Ad), FUN = sum, na.rm = TRUE, do_union = TRUE, simplify = TRUE)
# agg_union
# mapview(agg_union,zcol="Group.1") 

```

La fonction `aggregate()` a aggrégé l'attribut "population" des villes appartenant à la même région l'objet "villes_reg_pop" par leur région

La fonction

La géométrie est celle de x.
On a combiné les points ensemble = devenu multipoints.

by veut dire.

FUN veut dire (on aurait pu mettre mean)
na.rm veut dire.

by aurait pu être un autre objet spatial
...
```{r}
villes_reg_pop_CN<-subset(villes_reg_pop,Rgns_Ad=="Côte-Nord")
villes_reg_pop_CN
mapview(villes_reg_pop_CN)
pop_CN<-sum(villes_reg_pop_CN$population,na.rm = TRUE)
pop_CN
```
```{r}
test[6,]$population  # fonctionne
test[6,]$geometry    # fonctionne pas ... pas les memes points
```




<!-- Aggregate parcs regions  



Celui-ci fonctionne.
```{r}
test<-aggregate(parcs_reg["AREA"], by = list(parcs_reg$Rgns_Ad), FUN = sum, na.rm = TRUE)
mapview(test, zcol="Group.1")
```

```{r}
test2<-aggregate(parcs["AREA"], parcs_reg, FUN = sum, na.rm = TRUE)
test2
mapview(test2)
```
...
```{r}
test3<-aggregate(parcs_reg["AREA"], regions, FUN = sum, na.rm = TRUE)
test3
mapview(test3)
```

Celui-ci fonctionne aussi mais il faudrait régler le problème de la Mauricie?
```{r}
#Ceci fait l'equivalent de nz_avheight = aggregate(x = nz_height, by = nz, FUN = mean)
test4<-aggregate(parcs["AREA"],regions,FUN = sum, na.rm = TRUE )
test4
mapview(test4)
```
