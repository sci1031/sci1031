...
municipalites <- st_read("E:/ELF/Dropbox/TELUQ/ENSEIGNEMENT/Cours/SCI1031/Developpement/Structure_test/sci1031/Module7/data/villes/villes.shp")
...
st_layers("E:/ELF/Dropbox/TELUQ/ENSEIGNEMENT/Cours/SCI1031/Developpement/Structure_test/sci1031/Module7/data/parcs.gdb") 
...
parcs_nationaux <- st_read("E:/ELF/Dropbox/TELUQ/ENSEIGNEMENT/Cours/SCI1031/Developpement/Structure_test/sci1031/Module7/data/parcs.gdb", layer = "terpnq_s") 
...
Cette table d'attributs contient une diversité d'informations. L'attribut nommé `HAP_NM_TOP` contient le nom des municipalités. Notez que l’abréviation `TOP` signifie [toponyme](https://fr.wikipedia.org/wiki/Toponymie). 
...
Un autre type de manipulations fréquemment utilisé est celle d'enrichir un jeu de données vectorielles en lui ajoutant des attributs. Ces nouvelles informations peuvent provenir d'un autre *shapefile* ou d'une base de données non-spatiales.  

Nous allons voir dans cette sous-section comment fonctionnent les opérations `st_join()` et `merge()`.
...
Nous allons maintenant faire une jointure spatiale entre `villes` et `regions` afin qu'on associe à chaque municipalités sa région administrative.
...
Deux informations importantes méritent d'être mentionnées par rapport à la fonction `st_join()`:
1. Pour utiliser cette fonction les deux couches spatiales à joindre doivent être exprimées dans le même système de coordonnées de référence (SCR).
...
En fait, fait une intersection?

Visualisons le nouveau *shapefile* que nous venons de créer:
```{r}
mapview(villes_reg)
```

Notez que lorsque vous cliquez sur les points de la carte, la valeur de l'attribut pour "Rngs_Ad" est maintenant également donnée.

Nous pouvons aussi utiliser la fonction `join_st()` en vue de créer un filtre spatial, par exemple avec la fonction `subset()`:
...
Cette opération nous a permis de filtrer les villes du Québec pour retenir seulement celles situées dans la région administrative de la Côte-Nord.
...
pop <- read.csv("E:/ELF/Dropbox/TELUQ/ENSEIGNEMENT/Cours/SCI1031/Developpement/Structure_test/sci1031/Module7/data/villes/population.csv",encoding="UTF-8")
...
#### Opération spatiale sur les données vectorielles {-}
<!-- À FAIRE  -->
...
La fonction `st_simplify()` de la bibliothèque `sf` permet simplifier des objets vectoriels de types ligne ou polygone en réduisant le nombre de points que ceux-ci comprennent (souvenez-vous qu'une ligne est constituée d'une sucession de points, et un polygone d'un ensemble de lignes). 
