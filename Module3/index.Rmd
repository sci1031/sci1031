...
```{r load-libraries3, echo = FALSE, results='hide', warning = FALSE, message = FALSE}
#library(raster)
library(rgdal)
library(ggplot2)
library(dplyr)
...
# limites_terrestres <- st_read(nom_du_fichier)
# http://donnees.ville.montreal.qc.ca/dataset/polygones-arrondissements/resource/62f7ce10-36ce-4bbd-b419-8f0a10d3b280
limites_terrestres <- st_read("Module3/data/LIMADMIN.shp")
```
...
# pistes_cyclables <- st_read(nom_du_fichier)
pistes_cyclables <- st_read("Module3/data/LIMADMIN.shp")
...
accidents_velo <- st_read("Module3/data/LIMADMIN.shp")
```
...
Pour visualiser des lignes, nous n'avons pas besoin de définir l'argument `fill`. Vous remarquerez que les lignes, cette fois, sont vertes foncées (`darkgreen`).
