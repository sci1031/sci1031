...
* Question 1&nbsp;: quel est le mois disposant du plus faible niveau de pluviométrie pour le parc national possédant ce sommet?

* Question 2&nbsp;: où se situe le (ou les) point(s) le(s) plus chaud(s)? (La question porte sur la localisation spatio-temporelle du ou des points)
...
Nous allons maintenant utiliser ce que nous venons de voir pour répondre à la question suivante: 

* Quel est le mois disposant du plus faible niveau de pluviométrie pour le parc national possédant ce sommet?

Pour répondre à cette question, nous devons procéder ainsi :

1. Isoler les cellules de `pcp` qui correspondent à la zone du parc du Mont Mégantic.
1. Calculer la pluviométrie moyenne pour toutes les couches. 
1. Déterminer le mois et l'année pour lequel la pluviométrie moyenne est la plus faible. 


Pour la première étape, nous allons utiliser `mask()`. Mais ici, nous décidons de prendre les cellules de nos couches qui sont à l'intérieur du Parc du Mont-Mégantic ainsi que les cellules qui sont proches du parcs, dans un rayon de 5&nbsp;km. Cette opération de *buffer* est très souvent utilisée pour augmenter la couverture spatiale et ainsi obtenir une moyenne climatique à une plus large échelle. 



de pour isoler les cellules de `pcp` . Nous allons ajouter utiliser aussi faire une opération 

```{R}
megantic <- st_buffer(parcs[2,], dist = 5000)
```

Notons qu'il est possible de connaître l'unité avec `st_crs()`.
...
Nous allons maintenant appeler `mask()` pour extraire les cellules désirer (afin de pouvoir les visualiser mais notons que nous aurions pu utiliser seulement `extract()`)

```{R pcp_megantic}
pcp_megantic <- mask(pcp, st_transform(megantic, st_crs(pcp)))
```
...
Nous pouvons alors extraire avec `getValues()` et faire la moyenne couche par 
couche en utilisant `apply()` et `mean()` sans oublier d'enlever les valeurs non attribuées (`NA`).

```{R mean_pcp}
mean_pcp <- apply(getValues(pcp_megantic), 2, mean, na.rm = TRUE)
```

Et nous pouvons alors chercher le minimum de ces moyennes.
...
La couche `r which.min(mean_pcp)`, ce qui correspond au mois de novembre 2012. 



#### Réponse à la question 2

Pour répondre à la question 2, nous devons 

1. Isoler les cellules de `maxt` qui correspondent à la zone du parc du Mont Mégantic (pas de *buffer* cette fois).
2. Trouver la cellules pour laquelle cette valeur est maximale 


Comme nous avons maintenant l'habitude des opérations de `mask()`, nous allons faire cette première étape à l'aide d'une seule ligne de commande.


```{R}
maxt_megantic <- mask(maxt, st_transform(parcs[2, ], st_crs(maxt)))
```

Pour la seconde étape, il y a plusieurs façons de procéder. Nous allons procéder en deux temps&nbsp;: nous allons d'abord trouver la couche ou est le maximum puis trouver quelle est le point sur cette couche. 
...
La couche `r id` correspond au mois de Juillet 2010. Nous utilisons alors `xyFromCell()` pour avoir les coordonnées satiale de ces points, ce qui nous permet de répondre à notre questions.
...
Pour réaliser des profiles de températures, nous allons dans pour, nous allons réutiliser l'objet `megantic`
mais cette fois, simplement `extract()`, nous utiliserons ensuite la moyenne pour chaque couche. Nous pouvons faire une fonction comme vu plus haut. 

```{r prof_fun}
profile <- function(ras, mask) {
  apply(extract(ras, st_transform(mask, st_crs(ras)))[[1]], 2, mean, na.rm = TRUE)
}
```

Nous utiliserons les dates, nous pouvons utiliser celle de `mint` étant donné que ce sont les même dates pour les trois couches.


```{r profiles, warning = FALSE, fig.height = 9}
par(mfrow = c(3, 1))
plot(getZ(mint), profile(mint, megantic), type = "l")
plot(getZ(mint), profile(maxt, megantic), type = "l")
plot(getZ(mint), profile(pcp, megantic), type = "h")
```


#### Visualisation

Pour finir, nous allons animer en présentant construisant les donn.es. Nous allons commencer par 

1 carte par et une et ceux une date.


```{r fun_anim, warning = FALSE, fig.height = 9}
