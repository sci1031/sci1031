...
- Utiliser les objets de classe `SpatRaster`.
...
- `image_animate()`, `image_read()`, `image_write()`
...
Chacune des parties est structurée autour d'une problématique à résoudre. Dans la première partie, vous analyserez un jeu de données de 400 trajets de cyclistes à Montréal. Cette analyse vous amènera à manipuler des objets
...
Les données vectorielles et matricielles utilisées dans ce module peuvent être téléchargées en cliquant sur un seul lien: [données pour le module 9](https://github.com/sci1031/data/raw/main/Module9_donnees.zip). Sauvegardez le dossier compressé (`zip`) dans votre répertoire de travail `Module9_donnees` pour ce module, et dézippez-le. Le dossier comprend quatre fichiers:
...
Dans un premier temps, visualisons le trajet parcouru pour une période de temps arbitraire, autrement dit pour une période allant du premier point jusqu'à n'importe quels points $n$ le long du trajet `traj_long_pts`. À titre d'exemple, choisissons les 2000 premiers points.
...
tm_shape(lin, bbox = st_bbox(traj_long_pts)) +
  tm_lines(lwd = 2, col = "#CC00FF") +
  tm_title(paste0("temps: ", traj_long_pts$temps[2000], " / distance", st_length(lin)),
           size = 0.9)
```
<br>
...
Dans ce qui suit, nous allons structurer notre code avec quelques fonctions. Cela nous permet d'isoler les différentes étapes à réaliser et d'éviter de répéter du code. De manière générale, structurer son code en différentes fonctions permet d'augmenter la clarté du code et de diminuer les erreurs qui arrivent rapidement quand le code se complexifie. 
...
visual <- function(traj_pts, n = 100) {
  ...
  tm_shape(lin, bbox = st_bbox(traj_long_pts)) +
    tm_lines(lwd = 2, col = "#CC00FF") +
    tm_title(paste0("temps: ", traj_long_pts$temps[n], " / distance", st_length(lin)),
             size = 0.9)
}
```
<br>
...
visual <- function(traj_pts, n = 100, basemap = NULL) {
  ...
  tm_shape(lin, bbox = st_bbox(traj_long_pts)) +
    tm_lines(lwd = 2, col = "#CC00FF") +
    tm_title(paste0(txt_temps(tps), "  -  ", txt_distance(lin)), size = 0.9)
}
```
...
iii. La séquence peut générer des nombres décimaux, mais `visual()` demande des nombres entiers, nous utilisons alors `floor()` pour la conversion.

iv. Nous reprenons l'argument `basemap` pour qu'il puisse être passé à `visual()`.

v. La fonction `grid::grid.newpage()` de la bibliothèque `grid` (incluse avec `R`) efface la page graphique courante avant de dessiner la prochaine carte. Cela est nécessaire pour que chaque image soit capturée séparément lors de la création du GIF.
...
Une fois l'animation créée, pour un nombre d'images donné, nous voulons la sauvegarder en format GIF. Pour cela, nous allons générer chaque image individuellement avec `tmap_save()`, puis les assembler en GIF avec la bibliothèque `magick`.

<!-- Ajouter icone biblio  -->

Installons la bibliothèque `magick`.
```{r eval=FALSE}
install.packages("magick")
```
<br>

Nous générons d'abord chaque image individuellement avec `tmap_save()`. La fonction `sprintf()` permet de nommer les fichiers de façon ordonnée (`image_001.png`, `image_002.png`, etc.). Nous assemblons ensuite les images en GIF avec `image_animate()`, en spécifiant le nombre d'images par seconde (`fps`). Enfin, nous supprimons les images individuelles devenues inutiles avec `file.remove()`.

```{r, eval = FALSE}
library(magick)

images_dir <- "Module9/images"
idx <- floor(seq(2, nrow(traj_long_pts), length.out = 20))

for (k in seq_along(idx)) {
  tmap_save(visual(traj_long_pts, idx[k], NULL),
            filename = file.path(images_dir, sprintf("image_%03d.png", k)),
            width = 1000, height = 1000)
}

imgs <- image_read(sort(list.files(images_dir, pattern = "image_[0-9]+\\.png$",
                                   full.names = TRUE)))
image_write(image_animate(imgs, fps = 5),
            path = file.path(images_dir, "anim1.gif"))

file.remove(list.files(images_dir, pattern = "image_[0-9]+\\.png$",
                       full.names = TRUE))
```
...
```{r anim1, fig.align='center', out.width = "80%", echo=FALSE}
knitr::include_graphics('Module9/images/anim1.gif')
```

<br>

Pour finir, nous allons ajouter un fond de carte. Pour cela, nous allons télécharger un fond de carte de [OpenStreetMap](https://www.openstreetmap.org) grâce à la fonction `get_tiles()` de la bibliothèque [`maptiles`](https://CRAN.R-project.org/package=maptiles).

Installons la bibliothèque `maptiles`.
...
La fonction `get_tiles()` télécharge un fond de carte pour l'étendue d'un objet spatial et retourne un objet `SpatRaster`. Nous devons spécifier le fournisseur de tuiles (`provider`) et indiquer si on veut recadrer le résultat sur l'étendue de l'objet (`crop = TRUE`).

```{r maptiles, message = FALSE, warning = FALSE}
library(maptiles)
carte_fond <- get_tiles(traj_long_pts, provider = "OpenStreetMap", crop = TRUE)
carte_fond
```
...
images_dir <- "Module9/images"
idx <- floor(seq(2, nrow(traj_long_pts), length.out = 20))
fond <- tm_shape(carte_fond) + tm_rgb()

for (k in seq_along(idx)) {
  tmap_save(visual(traj_long_pts, idx[k], fond),
            filename = file.path(images_dir, sprintf("image_%03d.png", k)),
            width = 1000, height = 1000)
}
...
imgs <- image_read(sort(list.files(images_dir, pattern = "image_[0-9]+\\.png$",
                                   full.names = TRUE)))
image_write(image_animate(imgs, fps = 5),
            path = file.path(images_dir, "anim2.gif"))
...
Pour pouvoir répondre à ces questions et réaliser ces exercices, nous utiliserons différents *rasters* qui seront manipulés sous forme d'objets `SpatRaster` (mentionnés au [Module 5](#mat)). 
...
Les données préparées sont en format [NetCDF (Network Common Data Form)](https://fr.wikipedia.org/wiki/NetCDF). Ces données peuvent être importées en utilisant la fonction `rast()` de la bibliothèque `terra`, qui lit nativement le format NetCDF.

Chargeons la bibliothèque `terra` et importons les données&nbsp;:

```{r stack, message = FALSE, warning=FALSE}
library(terra)
# temperature minimale par mois pour
mint <- rast("Module9/Module9_donnees/mint.nc")
# temperature maximale par mois
maxt <- rast("Module9/Module9_donnees/maxt.nc")
# total des precipitations par mois
pcp <- rast("Module9/Module9_donnees/pcp.nc")
class(mint)
```

<br>

Il s'agit là d'objets de classe `SpatRaster` que nous détaillons dans la section suivante.



#### Les objets SpatRaster

Pour cette section, nous nous concentrons sur l'objet `mint`, notons cependant que tout ce que nous voyons ici s'applique aux autres objets `SpatRaster` que nous avons importés. Inspectons rapidement ce nouvel objet.
...
Nous obtenons des informations similaires à celles d'un objet de classe `SpatRaster` à une seule couche, cependant la ligne qui indique les dimensions possède un élément de plus indiquant le nombre de couches, et les valeurs affichées correspondent aux valeurs minimales et maximales pour chacune des différentes couches.


##### Extraire les informations générales {-}

Un objet `SpatRaster` multicouche est en fait une collection de couches qui
ont la même projection spatiale, la même résolution, la même étendue
spatiale et donc le même nombre de cellules. Pour obtenir ces informations individuellement, nous utilisons les mêmes fonctions que pour les `SpatRaster` à une seule couche.
...
Les dimensions du `SpatRaster` peuvent être aussi être affichées avec la fonction `dim()`
...
Comme nous pouvons le constater, 3 nombres sont retournés, un de plus que pour les objets `SpatRaster` à une couche, le dernier chiffre est le nombre de couches que nous avons mentionné plus haut, ici `r nlyr(mint)`. Ce nombre peut aussi être affiché en utilisant la fonction `nlyr()`.
...
L'objet `mint` est donc une collection de `r nlyr(mint)` couches, chaque couche représente des valeurs de températures minimales moyennes sur toutes la zone pour un mois donné. Ici, les couches sont ordonnées dans le temps (elles ont été combinées ainsi) et nous avons 1 couche par mois sur 10 ans.


##### Extraire une selection de couches {-}

Il est possible d'accéder à n'importe quelle couche en utilisant `[[`:
...
Nous pouvons aussi prendre une séquence d'indices et obtenir un autre `SpatRaster`. Par exemple, nous pouvons obtenir les données de la première année en utilisant les 12 premiers indices.
...
Il est aussi possible de combiner des couches du même `SpatRaster` et de différents `SpatRaster` (si les contraintes spatiales sont respectées) avec `c()`.

```{r}
c(mint[[1]], maxt[[1]], pcp[[1]])
```
<br>

Nous avons, ici, créé un `SpatRaster` en combinant les températures minimales et maximales, et les précipitations cumulées du mois de janvier de la première année (2007).
...
Nous obtenons ainsi une matrice de 12 colonnes (1 colonne par mois) et 10 lignes (1 par cellule). Il est aussi possible d'utiliser `values()` pour obtenir l'ensemble des valeurs d'un objet `SpatRaster`. Dans ce cas, les valeurs sont alors retournées sous forme de matrice.
...
Pour appliquer les mêmes opérations sur les différentes couches, il est possible d'utiliser la fonction `R` `apply()` après avoir utilisé la fonction `values()`. Par exemple, pour connaître la valeur minimale de chaque couche, nous pouvons utiliser la ligne suivante&nbsp;:
...
La matrice `val` a autant de lignes que de cellules et autant de colonnes que de couches. En indiquant `2` comme deuxième argument de `apply()`, on applique la fonction sur les colonnes, c'est-à-dire couche par couche.
...
Aussi, certaines fonctions ont des comportements prédéfinis pour les objets `SpatRaster`. Par exemple, la fonction `min()` retourne un objet de classe `SpatRaster` dont chaque cellule contient la valeur minimale des cellules des différentes couches ayant la même latitude et la même longitude (c'est-à-dire à la même position dans le *raster*).
...
Cette opération est équivalente à appliquer la fonction `apply()` et `quantile()` après avoir utilisé `values()`.
...
Il est aussi possible de faire des calculs sur les différentes couches en utilisant la fonction `app()`, par exemple nous pouvons obtenir les quantiles cellule par cellule de la manière suivante&nbsp;:
...
Ce qui nous donne un objet `SpatRaster` qui loge, sur chacune de ses 5 couches, les valeurs des différents quantiles pour chacune des combinaisons (longitude, latitude). Nous noterons alors qu'utiliser `app(mint, min)` revient à utiliser `min(mint)`.



##### Réaliser des opérations spatiales {-}

Les principales opérations spatiales pour les objets de classe `SpatRaster` multicouches utilisent les mêmes fonctions que pour les objets `SpatRaster` à une seule couche. Les opérations spatiales à réaliser seront alors, au besoin, répétées sur les différentes couches.

En guise de démonstration, nous allons utiliser les fonctions `project()`, `crop()`, `mask()` et `extract()` sur le `SpatRaster` `mint` (voir le [Module 5](#mat) et le [Module 8](#manip_mat) pour plus de détails sur ces fonctions).

Pour illustrer le changement de projections des `SpatRaster`, projetons `mint` en utilisant la projection de Mercator.

```{r reproj_stack,warning=FALSE}
library(sf)
mint_merc <- project(mint, st_crs(3857)$wkt)
mint_merc
```
<br>
...
```{r crop_stack}
mint_crop <- crop(mint, ext(c(-71.5, -71, 45, 45.5)))
```
...
# la fonction extract retourne un data.frame avec une colonne ID et une colonne par couche.
# Par exemple, retournons les 10 premières lignes et les 5 premières colonnes
val_frontenac[1:10, 1:5]
```
<br>

Notons qu'ici, cette opération aurait aussi pu être réalisée en utilisant `values()` sur `mint_mask`.


##### Visualiser des SpatRaster {-}

Comme pour les objets `SpatRaster` à une couche, nous pouvons rapidement visualiser les objets `SpatRaster` multicouches avec la fonction `plot()`.
...
La bibliothèque `tmap` en mode interactif permet également de visualiser les `SpatRaster` multicouches. Ci-dessous, nous visualisons les trois premières couches. Un trop grand nombre de couches rendra la carte difficile à interpréter; mieux vaut sélectionner un nombre limité de couches à visualiser.
...
La fonction `tmap_mode("view")` bascule `tmap` en mode interactif, ce qui produit une carte navigable (zoom, déplacement) basée sur `leaflet`, plutôt qu'une figure statique. Par défaut, `tmap` est en mode `"plot"` (figure statique). Remarquer que vous pouvez sélectionner l'affichage de l'une ou l'autre des couches dans le menu apparaissant dans le coin supérieur droit de la carte.


##### Lien entre SpatRaster et objets spatio-temporels {-}

Jusqu'ici nous avons implicitement utilisé un objet `SpatRaster` comme un objet
spatio-temporel:
...
Il est cependant important d'insister sur le fait qu'un `SpatRaster` ne contient pas nécessairement un objet spatio-temporel. La seule contrainte sur les couches contenues est spatiale : étendue, projection et résolution doivent être identiques. Ainsi, il est commun de travailler sur une même période avec des objets `SpatRaster` qui contiennent différentes variables pour une période donnée (par exemple, la température, les précipitations, l'élévation, etc.). Cela permet, entre autres, d'éviter la répétition des mêmes opérations sur plusieurs couches séparées. Aussi, certaines données sont, par nature, multicouches, c'est le cas des données d'imagerie satellitaire (on parle alors de bandes).
...
Il existe cependant un moyen de formaliser un peu plus l'information qui ordonne les couches avec la fonction `time()` de la bibliothèque `terra`. Nous allons utiliser cette fonction pour associer une date à chacune des couches contenues dans le `SpatRaster`.


En premier lieu, créons une séquence de date "mois/année", que nous appellerons `moan`, en utilisant les fonctions `R` `rep()` et `paste0()`:
...
Nous sommes alors en mesure d'ajouter cette information à `mint` en utilisant la fonction `time()`. Notons que la longueur de cette séquence doit correspondre au nombre de couches du `SpatRaster`, soit `nlyr(mint)` dans le cas présent, sinon un message d'erreur sera retourné.
...
Nous pouvons constater qu'une ligne "`time`" a été ajoutée, résumant cette nouvelle information. Ces données sont facilement accessibles avec `time()` qui nous retourne les valeurs de cet axe temporel.
...
La couche `mint` pour une année donnée le long de cet axe de temps est accessible en utilisant `[[`. Par exemple, la première couche est&nbsp;:
```{r}
mint[[1]]
```
<br>
...
```{r time_sort}
mint[[which(year(time(mint)) == 2015)]]
```
<br>
...
> 
> 1. Isoler les cellules du *SpatRaster* `pcp` qui correspondent à la zone du Parc national du Mont-Mégantic.
...
Nous allons maintenant appeler la fonction `mask()` de la bibliothèque `terra` pour extraire les cellules désirées du *raster* `pcp` (précipitations cumulées). Utiliser cette fonction nous permet de visualiser spatialement les données extraites, mais notons que nous aurions pu aussi utiliser la fonction `extract()` pour répondre à la question.
...
Notez également, que nous nous sommes assurés que le *raster* utilisé pour créer le masque soit dans le même système de coordonnées de référence que le `SpatRaster` interrogé.
...
Nous pouvons alors extraire les valeurs des différentes couches avec la fonction `values()` et faire la moyenne couche par couche (c'est-à-dire mois par mois) en utilisant les fonctions `R` `apply()` et `mean()`, sans oublier d'enlever les valeurs non attribuées (`NA`).

```{r mean_pcp}
mean_pcp <- apply(values(pcp_megantic), 2, mean, na.rm = TRUE)
```

<br>

Rappelez-vous que l'argument 2 signifie que la fonction `mean()` est appliquée sur la deuxième dimension de la matrice `values(pcp_megantic)` (c'est-à-dire la dimension associée aux couches).


Utilisons le vecteur `temps` créé plus tôt pour visualiser le changement dans les précipitations cumulées moyennes en fonction du temps&nbsp;:
...
Nous appliquons la fonction `mask()` au `SpatRaster` `maxt` qui contient les températures maximales moyennes en utilisant les limites du parc. Comme nous avons déjà réalisé des opérations de `mask()` à plusieurs reprises, nous allons faire cette première étape à l'aide d'une seule ligne de commande.
...
```{r}
id <- which.max(apply(values(maxt_megantic), 2, max, na.rm = TRUE))
id
```
<br>
...
et que cette température était de `r max(apply(values(maxt_megantic), 2, max, na.rm = TRUE))`&deg;C&nbsp;:
```{r}
max(apply(values(maxt_megantic), 2, max, na.rm = TRUE))
```
<br>


Utilisons maintenant la fonction `xyFromCell()` de la bibliothèque `terra` pour déterminer les coordonnées spatiales auxquelles cette température a été enregistrée.

```{r}
xyFromCell(maxt_megantic[[id]], which.max(values(maxt_megantic[[id]])))
```
<br>

Rappelons que la fonction `xyFromCell(A,id)` détermine les coordonnées (x,y) de la cellule d'un *raster* `A` dont l'indice est `id`. Ainsi, dans la ligne de code précédente, le premier argument de la fonction `xyFromCell()` correspond au *raster* au sein duquel on recherche les coordonnées (x,y). Dans le cas présent, ce *raster* est la couche `r id` du `SpatRaster` `maxt_megantic`. Le deuxième argument correspond à l'indice de la ou des cellules qui contiennent le maximum. Dans `terra`, `which.max()` appliqué directement à un `SpatRaster` retourne un `SpatRaster`; on utilise donc `which.max(values(...))` pour obtenir l'indice de cellule souhaité.
...
Pour réaliser ces profils, nous allons faire une moyenne pour chaque couche des valeurs puis les afficher en fonction du temps. Nous allons répéter trois fois la même opération ici (une fois pour chaque `SpatRaster`: `maxt.nc`, `mint.nc` et `pcp.nc`). Au lieu de copier/coller notre code pour les trois sources de données, nous allons créer une fonction `profil()`. Cette fonction prend en intrant `ras`, un objet `SpatRaster` et `masque`, l'objet spatial qui sera utilisé pour découper le *raster*.
...
profil <- function(ras, masque) {
  ...
  ras_extract <- extract(ras, vect(masque_crs))
  # notons que la fonction `extract()` retourne un data.frame dont la première
  # colonne est l'identifiant du polygone; on la retire avec [, -1]
  # calculer la moyenne pour chaque couche
  apply(ras_extract[, -1], 2, mean, na.rm = TRUE)
}
```

<br>

Cette fonction nous permet donc d'obtenir les valeurs environnementales moyennes à mettre en ordonnées de nos profils. Il nous reste à obtenir les valeurs des abscisses, autrement dit les dates pour chacune des moyennes. Comme les dates sont les mêmes pour `mint`, `maxt` et `pcp`, nous utilisons `time(mint)` qui nous donne exactement ce dont nous avons besoin. Notez que nous pouvons également utiliser le vecteur `temps` créé plus tôt.
...
plot(time(mint), profil(mint, megantic_buf), 
  type = "l", ylab = "Températures minimales (°C)", xlab = "")
plot(time(mint), profil(maxt, megantic_buf), 
  type = "l", ylab = "Températures maximales (°C)", xlab = "")
plot(time(mint), profil(pcp, megantic_buf), 
...
utiliser les températures minimales et maximales moyennes pour créer une enveloppe de températures. Cela est possible en faisant appel à la fonction `polygon()`. Attention, la fonction `polygon()` n'est pas liée à la fonction `st_polygon()`!
...
Finalement, nous sauvegardons une séquence de figures produites avec `plot_profil()` en faisant évoluer le nombre de mois. Comme `plot_profil()` est une figure base R (et non `tmap`), nous utilisons la fonction `png()` pour sauvegarder chaque image, puis `magick` pour les assembler en GIF&nbsp;:
...
for (k in seq_along(idx)) {
  png(file.path(images_dir, sprintf("image_%03d.png", k)), width = 600, height = 400)
  plot_profil(idx[k], pr_mint, pr_maxt, pr_pcp, temps)
  dev.off()
}
...
imgs <- image_read(sort(list.files(images_dir, pattern = "image_[0-9]+\\.png$",
                                   full.names = TRUE)))
image_write(image_animate(imgs, fps = 5),
            path = file.path(images_dir, "anim3.gif"))
...
Pour commencer, il nous faut sélectionner la région demandée de `mint` en utilisant la fonction `crop()`. Nous savons que ce *SpatRaster* utilise le système WGS84, nous n'avons donc pas à changer le SCR.

```{r}
region <- ext(c(-71.5, -71, 45, 45.4))
mint_region <- crop(mint, region)
```
<br>

Ensuite, construisons une fonction pour afficher une seule couche du *SpatRaster* `mint_region` avec les caractéristiques demandées. Définissons d'abord les éléments propres à la légende, aux couleurs, et au titre.
...
fct_carte <- function(RS,t){
  tm_shape(RS[[t]]) + #couche correspondant au mois t
    tm_raster(col.scale = tm_scale(values = Coul, breaks = L, labels = noms),
              col.legend = tm_legend(title = "",
                                     orientation = "portrait",
                                     reverse = TRUE,   #assure que les T négatives sont au bas de la légende
                                     position = tm_pos_out("right", "center"),
                                     text.size = 0.8)) +
    tm_title(paste0("Températures minimales (°C) - ", FR[month(temps[t])], " ", year(temps[t])),
             size = 0.9) +
    tm_layout(legend.format = list(text.align = "right"))
}
```
<br>
...
Vous pouvez également sauvegarder cette animation en exécutant ces lignes de commande&nbsp;:
```{r, eval = FALSE}
library(magick)
...
for (k in seq_along(idx)) {
  tmap_save(fct_carte(mint_region, idx[k]),
            filename = file.path(images_dir, sprintf("image_%03d.png", k)),
            width = 1000, height = 1000)
}
...
imgs <- image_read(sort(list.files(images_dir, pattern = "image_[0-9]+\\.png$",
                                   full.names = TRUE)))
image_write(image_animate(imgs, fps = 5),
            path = file.path(images_dir, "anim_mint.gif"))
