...
par(mfrow = c(1, 2)) # figure a deux panneaux cÃ´te à cÃ´te
hist(trajets$length, 
...
<!-- pas sÃ»r de comprendre l'idée derrière ce qu'on fait  -->
...
GrÃ¢ce à l'étape précédente, nous pouvons facilement avoir accès à l'heure correspondant au dernier point de ce trajet. 
...
Pour finir, nous allons ajouter un fond de carte. Pour cela nous allons télécharger un fond de carte de [OpenStreetMap](https://www.openstreetmap.org) grÃ¢ce à fonction `osm.raster()` de la bibliothèque [`rosm`](https://CRAN.R-project.org/package=rosm).
...
* Question 2&nbsp;: oÃ¹ se situe le (ou les) point(s) le(s) plus chaud(s) autour du parc national du Mont-Mégantic? Cette question porte sur la localisation spatio-temporelle du ou des points les plus chauds.
...
* longitude : de 72Â°Ouest à 71Â°Ouest,
* latitude : 45Â°Nord à 46Â°Nord. 
...
* longitude : de 71.5Â°Ouest à 71Â°Ouest,
* latitude : 45Â°Nord à 45.5Â°Nord. 
...
<!-- pas sÃ»r si panel est français  -->
...
plot(getZ(mint), profil(mint, megantic_buf), 
  type = "l", ylab = "Températures minimales (Â°C)", xlab = "")
plot(getZ(mint), profil(maxt, megantic_buf), 
  type = "l", ylab = "Températures maximales (Â°C)", xlab = "")
...
plot_profil <- function(n, pr_mint, pr_maxt, pr_pcp, temps) {
  ...
  plot(range(temps), c(min(pr_mint), max(pr_maxt)), type = "n", 
    xlab = "Années", ylab = "Températures minimales et maximales (Â°C)")
  # créer une enveloppe 
