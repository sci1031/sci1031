...
Pour pouvoir faire tout ce travail, nous allons utiliser différents *rasters* qui seront manipulés sous forme d'objets `rasterStack` (mentionnés au [Module 8][#mat]) qui seront l'objet des principaux objectifs d'apprentissage de cette section.
...
Pour la seconde étape, il y a plusieurs façons de procéder. Ici, nous allons procéder en deux temps&nbsp;: nous allons d'abord trouver la couche (ou les couches) qui contient (contiennent) la valeur de température maximum, puis trouver les coordonnées du point (ou des points) associées. Nous calculons donc le maximum pour chaque couche, puis nous en déterminons quel est le maximum de ces maximums.
...
Nous obtenons ainsi une fonction qui nous donnes les valeurs à mettre en ordonnées de nos profile. Il nous reste à obtenir les valeurs des abscisses, autrement dit les dates pour chacune des moyennes. Comme les dates sont les même pour `mint`, `maxt` et `pcp`, nous utilisons `getZ(mint)` qui nous donne exactement ce dont nous avons besoin. 
...
Pour finir, nous allons animer les profiles que nous venons de dessiner. Nous allons commencer par améliorer quelque peu le rendu visuel. Pour cela nous allons: 
...
Nous allons encore une fois structurer notre code avec une fonction. Nous commencer par créer `plot_profile()` qui prend en entrée `n`, le nombre de mois à afficher, les différents profiles ainsi que la séquence de dates. Ici, nous ne calculons pas les profiles dans la fonction pour ne pas les recalculer à chaque fois que nous créons une nouvelle figure, cela ajouterait un temps de calcul important inutilement, nous les calculerons en amont, une fois pour toutes.


```{R plot_profile}
plot_profile <- function(n, pr_mint, pr_maxt, pr_pcp, temps) {
  # indices de 1 à n
  id <- seq_len(n)
  # premier passage, les temperatures 
  par(mar = c(4, 4.5, 1, 4.5), las = 1)
  # créé une figure vide (type = "n") avec les dimension 
...
Il nous reste maintenant à faire les profiles une fois pour toute.
...
Nous passons alors ces profiles à `plot_profile()` et pour visualiser le profile pour les 40 premiers mois (par exemple), nous utilisons la valeur `40` pour `n`.   

```{R plot_profile_40}
plot_profile(40, pr_mint, pr_maxt, pr_pcp, temps)
```

Nous utilisons alors la fonction `saveGIF()` du package `animation`, et avec une séquence de figures créée avec `plot_profile()`&nbsp;: 
