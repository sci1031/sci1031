...
Téléchargez les [données du MEES](https://github.com/sci1031/sci1031/tree/master/Module4/data/Donnees_Ouvertes_MEES.zip). Sauvegardez le dossier compressé (`zip`) dans votre répertoire de travail `Donnees` pour ce module, et dézippez-le. Le dossier Donnees_Ouvertes_MEES comprend un seul sous-dossier, Donnees_Ouvertes_MEES.gdb, c'est la geodatabase qui elle contient de multiples fichiers de formats différents.
...
chemin<-"D:/votrechemin/SCI1031/Module4/Donnees/"
nom_du_fichier <- paste(chemin, "/Donnees_Ouvertes_MEES/Donnees_Ouvertes_MEES.gdb", sep = "")
couches_mees <- st_layers(nom_du_fichier)
couches_mees
```

```{r mees-layers, eval = TRUE, echo = FALSE}
couches_mees <- st_layers("Module4/data/Donnees_Ouvertes_MEES/Donnees_Ouvertes_MEES.gdb")
couches_mees
```
...
chemin<-"D:/votrechemin/SCI1031/Module4/Donnees/"
nom_du_fichier <- paste(chemin, "/Donnees_Ouvertes_MEES/Donnees_Ouvertes_MEES.gdb", sep = "")
ecoles_public <- st_read(nom_du_fichier, layer = "PPS_Public_Ecole")
```

```{r ecoles_pub, eval = TRUE, echo = FALSE}
ecoles_pub <- st_read("Module4/data/Donnees_Ouvertes_MEES/Donnees_Ouvertes_MEES.gdb", layer = "PPS_Public_Ecole")
```
...
#### Explorer les métadonnées d'une couche {-}

Explorons les attributs associés
