...
La première section^[chapitre sur les données vectorielles du manuel [Geocomputation with R](https://bookdown.org/robinlovelace/geocompr/spatial-class.html) de Robin Lovelace, Jakub Nowosad et Jannes Muenchow] 
...
Nous avons appris à la [leçon 2](#base) qu'il existe différents types de données vectorielles, dont les points, les lignes, et les polygones. La bibliothèque `sf` possède des fonctions pour créer ces géométries simples. 

La fonction `st_point()` permet de transformer un vecteur numérique de coordonnées $(x,y)$ en une donnée vectorielle de type *point*. Par exemple
...
Remarquez que la classe de l'objet formé est `sfg`, ce qui signifie "simple feature geometries" :
```{r}
class(point_x)
```
<br>

Pour créer des données vectorielles *multipoints*, nous devons d'abord créer une matrice contenant un ensemble de points, puis convertir cette matrice en utilisant la fonction `st_multipoint()`:
...
Pour créer une *ligne*, nous devons également créer une matrice contenant les coordonnées des extrémités de la ligne. On convertie alors cette matrice avec la fonction `st_linestring()`:
...
```{r}
image des points, ligne, polygone.
```
...
Nous venons d'apprendre les fonctions de base de la bibliothèque `sf` pour créer des géométries simples. Des données vectorielles doivent aussi être géoréférencées. Ceci signifie qu'on doit leur attribuer un datum et une projection. 

```{r}
multi_point <- CRS(multi_point, 26919)
```


### Télécharger les données {#data_mod4}

Afin de faciliter le téléchargement des multiples données utilisées dans ce module, l'ensemble des couches d'informations spatiales peuvent être téléchargées en cliquant sur un seul lien: [données pour le module 4](https://github.com/elisefilotas/Donnees_spatiales/blob/master/Module4_donnees.zip). Sauvegardez le dossier compressé (`zip`) dans votre répertoire de travail `Module4_donnees` pour ce module, et dézippez-le. Le dossier comprend lui même deux dossiers compressés: 

- `Montreal_Velo.zip`
- `Donnees_Ouvertes_MEES.gbd.zip`


Dézipper chacun des deux dossiers. Le premier dossier, `Montreal_Velo` contient les données vectorielles relatives au réseau de pistes cyclables de la ville de Montréal et aux accidents routiers impliquant des bicyclettes. Il contient trois sous-dossiers: 

- `accidents`
- `pistes`
- `terre`.

Le deuxième dossier, `Donnees_Ouvertes_MEES.gbd` est la geodatabase du Ministère de l'Éducation et de l'Enseignement Supérieur du Québec (MEES); nous l'utiliserons à la section [7.4](#gdb).
