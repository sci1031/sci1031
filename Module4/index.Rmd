...
```{r, eval = FALSE}
install.packages(c('sf', 'rgdal', 'mapview', 'leafsync'))
```
...
Dans les sections [4.2](#read_vec) à [4.5](#gdb) du présent module vous apprendrez à lire et visualiser des données déjà existantes. Afin de faciliter le téléchargement de ces multiples données, l'ensemble des couches d'informations spatiales peuvent être téléchargées en cliquant sur un seul lien: [données pour le module 4](https://github.com/elisefilotas/Donnees_spatiales/blob/master/Module4_donnees.zip). Sauvegardez le dossier compressé (`zip`) dans votre répertoire de travail `Module4_donnees` pour ce module, et dézippez-le. Le dossier comprend lui même deux dossiers compressés: 

- `Montreal_Velo.zip`
- `Donnees_Ouvertes_MEES.gbd.zip`


Dézipper chacun des deux dossiers. Le premier dossier, `Montreal_Velo` sera utilisé aux sections [4.2-4.4](#read_vec). Il contient les données vectorielles relatives au réseau de pistes cyclables de la ville de Montréal et aux accidents routiers impliquant des bicyclettes. Il contient trois sous-dossiers: 

- `accidents`
- `pistes`
- `terre`.

Le deuxième dossier, `Donnees_Ouvertes_MEES.gbd` est la geodatabase du Ministère de l'Éducation et de l'Enseignement Supérieur du Québec (MEES); nous l'utiliserons à la section [4.5](#gdb).



### Créer des données vectorielles {#create_vec}

Pour créer, lire et manipuler des données vectorielles, nous allons utiliser la librairie `sf`. Notez que la librairie `rgdal` se charge automatiquement lorsque `sf` se charge.
...
```{r plotgeo, eval = TRUE, echo = FALSE, fig.cap = "Géométries multipoint, ligne et polygone crées avec les fonctions `st_multipoint()`,`st_linestring()`, et `st_polygon()` respectivement."}
mapviewOptions(homebutton = FALSE)
m_multi_point <- mapview(multi_point, homebutton =  FALSE)
...
```{r pistes, eval = TRUE, echo = FALSE, comment=""}
...
```{r plot-terre, echo = TRUE,  warning = FALSE, message = FALSE, eval = TRUE}
...
```{r plot-velo, warning = FALSE, message = TRUE, fig.cap = "Pistes cyclables sur l'île de Montréal"}
...
```{r plot-voie0, echo = TRUE,  warning = FALSE, message = FALSE, eval = TRUE, attr.output='style="max-height: 200px;"'}
pistes_cyclables
```
<br>
...
```{r  plot-viridis, echo = FALSE,  warning = FALSE, message = FALSE, eval = TRUE, asp = 1, fig.show='hold', fig.align='center', fig.cap="Palette viridis contenant 20 couleurs différentes"}
library(viridis)
library(munsell)
library(colorspace)
...
```{r plot-voie4, echo = TRUE, warning = FALSE, message = FALSE, fig.cap="Types de voie cyclable sur l'île de Montréal"}
...
Représentons mainteant les deux projections différentes en utilisant la fonction `latticeView()` de la bibliothèque `leafsync` (Fig \@ref(fig:plot-terre2)). Puisque cette fonction existe à la fois dans la bibliothèque `leafsync` et dans la bibliothèque `mapview` mais qu'elle est obsolète dans cette dernière, nous devons préciser que nous voulons la fonction latticeView()` de la bibliothèque `leafsync` en utilisant la notation suivante: `leafsync::latticeView`
...
```{r plot-terre2, echo = TRUE,  warning = FALSE, message = FALSE, eval = TRUE}
...
```{r autres-institutions, eval = TRUE, echo = FALSE, message = FALSE, results = FALSE}
...
Pour cette question, vous utiliserez les données vectorielles décrivant les régions administratives de la Nouvelle-Zélande. Ces données se nomment `nz` et sont disponibles dans la bibliothèque 
...
Pour répondre à cette question, nous pouvons simplement repérer l'information demandée dans l'affichage générale de l'objet `nz`
```{r}
nz
```
<br>
...
Effectivement, `nz` étant un `data.frame`, nous pouvons utilisé les manipulations usuelles pour cette classe d'objet. Chaque élément dans un `data.frame` occupe une rangé. 


```{r}
st_geometry_type(nz)
```
La couche `nz` contient bien des multipolygones. 

**b)**  Trouver le nombre d'attributs de `nz` et leur nom.

Pour trouver le nombre d'attributs, nous pouvons nous référer à l'affichage général de `nz` qui nous informe que l'objet contient 6 attributs (ou champs, *fields* en anglais).
...
L'objet `nz` contient bel et bien 7 colonnes. Cependant, la dernière colonne ne correspond à la géométrie de chaque élément de `nz` (qui est en fait, un attribut spatial). Ainsi, le nombre d'attribut est 6.
...
```
**f)** Le fichier `nz_capitales.cvs` donne le nom et la localisation des capitales de chaque région de la Nouvelle-Zélande. Créer une carte illustrant les frontières des régions et la position de leur capitale respective.


Tout d'abord, vous devez importer le fichier `nz_capitales.cvs` dans votre session de travail `R`. Utilisant la fonction de base `read.table()` afin de stocker les coordonnées des capitales dans un object de classe `data.frame`.



```{r fake-capnz, eval = FALSE}
nom_du_fichier<-"D:/votrechemin/SCI1031/Module4/Module4_donnees/nz_capitales.csv"
nz_capitales <- read.table(nom_du_fichier, header = TRUE, sep = ",")
nz_capitales
```

```{r capnz, eval = TRUE, echo = FALSE, comment=""}
...
Utilisons la fonction `mapview()` pour visualiser les polygones des régions et les points correspondants aux capitales de ces régions.
