...
```{r}
nz_cities <- read.table("Module4/nz_cities.csv", sep = ",")
capitales <- c("Auckland", "Whangarei", "Hamilton", "Whakatāne", "Gisborne", "Napier", "Stratford", "Palmerston North", "Wellington", "Richmond", "Nelson","Blenheim","Greymouth","Christchurch", "Dunedin", "Invercargill")

nz_capitales <- nz_cities[nz_cities$V1 %in% capitales,]
nz_capitales <- nz_capitales[,c("V1", "V2","V3")]
names(nz_capitales) <- c("Capitales", "Lat", "Long" )
coord_cap <- nz_capitales[, c("Lat","Long")]
coord_cap_mat <- coord_cap
names(coord_cap_mat) <- NULL
coord_cap_mat <- as.matrix(coord_cap_mat, nrow = nrow(cap_coord), row.names = NULL)
coord_cap_num <- as.numeric(coord_cap_mat)
coord_cap_mat2 <- matrix(coord_cap_num, ncol = 2)

#reconvert to dataframe to save as csv ??
st_point() etc
```
...
st_crs(nz)$epsg
#nz_lcc <- st_transform(nz, crs = 32198)
#nz_nad83 <-  st_transform(nz, crs = 32188)
nz_wgs84 <- st_transform(nz, crs = 4326)
```
```{r}
m1 <- mapview(nz_wgs84,native.crs=TRUE)
m2 <- mapview(nz)
