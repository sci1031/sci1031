...
La bibliothèque `tmap` comprend plusieurs fonctions permettant de représenter des données de type point. Familiarisons-nous d'abord avec les fonctions `tm_dots()` et `tm_symbols()`. Nous verrons plus loin les fonctions `tm_bubbles()` et, dans la section sur les cartes interactives, `tm_markers()`.
...
```{r, echo = FALSE, fig.cap = "Formes disponibles dans R et leur numéro (`shape`).", fig.height=3, fig.width=7, fig.align="center"}
par(mar = c(0, 0, 0, 0))
...
La fonction `tm_markers()` est disponible en mode interactif pour placer des marqueurs sur la carte. Reprenons les données de municipalités `V` :

```{r, eval = FALSE, echo = TRUE}
tm_shape(Q) +
  tm_polygons(fill = "grey85") +
tm_shape(V) +
  tm_markers(id = "Mncplts")
```
