...
où l'argument `breaks` précise les divisions sur la barre d'échelle, `text.size` la taille du texte sous la barre, et `position` la position de la barre sur la carte, spécifiée avec `tm_pos_in("horizontal", "vertical")` où `"horizontal"` est `"left"`, `"center"` ou `"right"`, et `"vertical"` est `"top"`, `"center"` ou `"bottom"`.
...
Plusieurs options d'arguments existent pour les fonctions `tm_grid()` et `tm_graticules()`. Par exemple, il est possible de préciser le nombre de divisions sur l'axe des *x* (`n.x`) et sur l'axe des *y* (`n.y`)^[Noter que le nombre de divisions créé par `tmap` est approximativement celui demandé car `tmap` crée des divisions uniformément espacées situées sur des coordonnées de valeurs entières.], l'épaisseur du trait (`lwd`), la couleur (`col`) ou encore la taille de l'écriture (`labels.size`). Dans l'exemple ci-dessous, `tm_graticules()` est appelé sans préciser `crs`, ce qui utilise le CRS par défaut (EPSG:4326, WGS84) plutôt que NAD83 (EPSG:4269) — notez la légère différence dans le positionnement des graticules.
...
* L'ajout d'un titre (`main.title`), la taille de ce dernier (`main.title.size`) et sa position (`main.title.position`).
...
map_Q + tm_layout(main.title = "Carte du Québec", main.title.size = 0.8, main.title.position = c("right", "top"))
...
Qtitre = map_Q + tm_layout(main.title = "Carte du Québec", main.title.size = 0.8, main.title.position = c("right", "top"))
Qframe = map_Q + tm_layout(frame = FALSE)
Qbg = map_Q + tm_layout(bg.color = "aquamarine", scale = 2)
Qf = map_Q + tm_layout(frame.double.line = TRUE, outer.margins = c(0, 0.2, 0, 0.2))
...
tm_shape(Q) + tm_fill() +
  ...
  tm_layout(main.title = "Réseau routier")
```
<br>
