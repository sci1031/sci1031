...
Note: dire quelquechose à propos de https://cols4all.github.io/cols4all-R/
-->
...
La fonction `tm_legend()` permet de configurer la position de la légende. Les arguments utiles sont :
...
tm_shape(Q) +
  tm_polygons(fill = "NUM_REG",
              fill.legend = tm_legend(title = "Régions administratives",
                                      ...
                                      position = tm_pos_out("bottom", "center"))) +
...
```{r eval = FALSE, echo = TRUE}
tm_shape(Q) +
  tm_polygons(fill = "NOM_REG", fill.legend = tm_legend(title = "Régions administratives")) +
...
La fonction `tm_layout()` permet de configurer l'apparence de la légende. Les arguments utiles sont :

* L'option de mettre un cadre autour de la légende (`legend.frame`) et la couleur de fond de la légende (`legend.bg.color`).
* La taille de la légende extérieure (`legend.outside.size`) et la taille du texte de la légende (`legend.text.size`).
* La police de caractère (`legend.title.fontfamily`), le style (`legend.title.fontface`), et la couleur (`legend.title.color`) du titre de la légende.
...
Les couleurs des éléments d'une carte se définissent directement dans les fonctions de couche plutôt que dans `tm_layout()`. Certains arguments de `tm_layout()` permettent néanmoins d'ajuster l'apparence globale des couleurs:

* La couleur de remplissage d'un polygone se définit avec l'argument `fill` et la couleur des frontières avec l'argument `col` dans `tm_polygons()`.
* L'argument `saturation` de `tm_layout()` défini le niveau de saturation des couleurs. La valeur par défaut est 1, et la valeur 0 donne une représentation en noir et blanc. Il est possible de donner des valeurs supérieures à 1 pour des couleurs très saturées.
* L'argument `sepia.intensity` de `tm_layout()` est un nombre entre 0 et 1 qui défini le niveau de "chaleur" des couleurs. Plus sa valeur est grande, plus les couleurs ont une teinte jaune voir brune. La valeur par défaut est 0.
* La palette de couleurs d'une couche se change avec `fill.scale = tm_scale(values = "nom_palette")`. La bibliothèque `tmap` utilise les palettes de couleurs de [Color Brewer](https://colorbrewer2.org/). Dans le cas d'attributs catégoriques (comme le nom de régions) la palette utilisée par défaut se nomme `Set3`, mais il est possible de choisir d'autres palettes parmi celles-ci: `Accent`, `Dark2`, `Paired`, `Pastel1`, `Pastel2`, `Set1`, et `Set2`. Nous reviendrons sur le sujet des palettes un peu plus loin dans cette leçon.
...
Q1 <- tm_shape(Q) + tm_polygons(fill = "lightblue", col = "darkgreen")
Q2 <- tm_shape(Q) + tm_polygons(fill = "NUM_REG", col = "white") +
                    tm_layout(legend.show = FALSE, saturation = 0)
Q3 <- tm_shape(Q) + tm_polygons(fill = "NUM_REG") +
...
tm_shape(Q) + tm_polygons(fill = "black", col = "white") +
                ...
                tm_text("NUM_REG", col = "NOM_REG",
                        col.scale = tm_scale(values = "Paired"),
...
Q2 <- tm_shape(Q) + tm_fill(fill = "blue", fill_alpha = 0.4) +
...
tm_shape(V) + tm_dots(fill = "Mncplts",
                      fill.scale = tm_scale(values = "Paired"),
                      size = 1) +
              tm_layout(frame = FALSE,
                        legend.outside = TRUE,
                        legend.title.color = NA,
                        legend.text.size = 0.8) +
              tm_title("Municipalités")
```
<br>

###### Fonction tm_markers() {-}

La fonction `tm_markers()` représente les points par le symbole de repère géographique. Elle fonctionne en mode interactif (voir la section [Cartes interactives](#cartes-interactives)) :
...
```{r, eval = FALSE, echo = TRUE}
tm_shape(E) + tm_raster(col.legend = tm_legend(title = "Élévation (m)"))
tm_shape(E) + tm_raster(col.scale = tm_scale(n = 10),
                        col.legend = tm_legend(title = "Élévation (m)"))
```
```{r, echo = FALSE, dev = "png", fig.height=6 }
Q1 <- tm_shape(E) + tm_raster(col.legend = tm_legend(title = "Élévation (m)"))
Q2 <- tm_shape(E) + tm_raster(col.scale = tm_scale(n = 10),
                               col.legend = tm_legend(title = "Élévation (m)"))
...
```{r, dev = "png", fig.height=6 }
tm_shape(E) + tm_raster(col.scale = tm_scale(n = 10),
                        col.legend = tm_legend(title = "Élévation (m)", hist = TRUE)) +
              tm_layout(legend.outside = TRUE)
```
<br>
...
tm_shape(E) + tm_raster(col.scale = tm_scale(n = 10, values = pal.elevation(10),
                                             value.na = "beige"),
                        col.legend = tm_legend(title = "Élévation", hist = TRUE)) +
              tm_layout(legend.outside = TRUE)
```
<br>

Il existe différentes façons de former des classes de valeur à partir de la distribution. L'argument `style` se précise à l'intérieur de la fonction `tm_scale_intervals()` assignée à l'argument `col.scale` de `tm_raster()`. Il permet de choisir une méthode parmi plusieurs dont les suivantes:
...
tm_shape(Q) +
  tm_polygons(fill = "NOM_REG",
   ...
   tm_symbols(fill = "black",
              col = "grey",
              size = "Pop_tot",
              size.scale = tm_scale(max.size = 2),
...
tm_shape(Q) +
  tm_polygons(fill = "NUM_REG",
              fill.scale = tm_scale(values = "Greys"),
              fill.legend = tm_legend(show = FALSE)) +
   tm_bubbles(size = "Pop_tot",
              ...
              size.legend = tm_legend(title = "Population (en milliers)",
                                      orientation = "landscape",
                                      labels = c("500", "1000", "1500", "2000", "2500")),
              fill.legend = tm_legend(title = "Proportion d'enfants (0-14 ans)")) +
    tm_layout(frame = FALSE,
              legend.outside = TRUE,
              legend.title.size = 1)
```
<br>
L'argument `style` se précise à l'intérieur de `fill.scale = tm_scale_intervals(style = "quantile")` pour définir les classes de couleurs. Ici, nous avons choisi une classification en quantile. Noter que nous pouvons définir un titre pour la légende des tailles (`size.legend = tm_legend(title = ...)`) et un titre pour la légende des couleurs (`fill.legend = tm_legend(title = ...)`).
...
Pour créer des cartes choroplèthes, nous utilisons l'argument `fill` de la fonction `tm_polygons()`. L'assignation des couleurs se fait en utilisant `fill.scale = tm_scale_intervals()` pour définir les classes de valeur d'attribut, de la même façon que pour `tm_raster()` et `tm_bubbles()`.
...
E_Outaouais <- crop(E, ext(Q_Outaouais))
