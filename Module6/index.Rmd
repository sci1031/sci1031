...
# transparence
Q2 <- tm_shape(Q) + tm_fill(fill = "green", alpha = 0.4)
...
Q6 <- tm_shape(Q) + tm_fill(fill = "blue", alpha = 0.3) +
...
Par exemple, la couleur d'un polygone peut représenter la valeur d'un de ses attributs. Dans ce cas, nous utilisons la fonction `tm_fill()` en associant à l'argument `fill` le nom de l'attribut que nous désirons illustrer.

```{r, dev = "png", fig.height=6}
tm_shape(Q) + tm_fill(fill = "NUM_REG", fill.legend = tm_legend(title = "Régions"))
```
...
tm_shape(Q_Outaouais) + tm_fill(fill = "blue", alpha = 0.4)
```
<br>
...
Q2 <- tm_shape(Q_Outaouais) + tm_fill(fill = "blue", alpha = 0.4)
Q12 <- Q1 + Q2
...
```{r eval = FALSE, echo = TRUE}
tm_shape(Q) +
  tm_fill(fill = "NOM_REG", fill.legend = tm_legend(title = "Régions administratives")) +
...
* La couleur de remplissage d'un polygone se définit avec l'argument `fill` et la couleur des frontières avec l'argument `col` dans `tm_polygons()` ou `tm_fill()`.
...
Q2 <- tm_shape(Q) + tm_fill(fill = "blue", alpha = 0.4) +
