...
- Utiliser les fonctions de `tmap` pour créer des cartes interactives
...
Dans la section [leçon](#lecon_carto), vous utiliserez quatre ensembles de données différents. Trois ensembles sont formés de données vectorielles. Il s'agit de données sur les régions administratives du Québec et la taille de leur population (polygones), des routes du Québec (lignes) et des municipalités du Québec (points). L'ensemble de données matricielles correspond à des données d'élévation sur tout le territoire québécois.
...
<br>
```{r teinte,  fig.cap="Un exemple de 8 différentes teintes de couleur", echo=FALSE, message = FALSE, fig.height=0.5, fig.width = 4,fig.show="hold",fig.align="center"}
# SOURCE: code from Gimond: https://github.com/mgimond/Spatial/blob/main/04_symbolizing_features.qmd
library(munsell)
library(colorspace)
...
# SOURCE: code adapted from Gimond: https://github.com/mgimond/Spatial/blob/main/04_symbolizing_features.qmd
...
mapCont <- tm_shape(World2) +
  tm_polygons(fill = "continent",
              fill.scale = tm_scale(values = pal,
                                    labels = c("Afrique", "Antartique", "Asie", "Europe", "Amérique du Nord", "Océanie", "Amérique du Sud"),
                                    drop.levels = TRUE),
...
mapPop <- tm_shape(World) +
  tm_polygons(fill = "pop_est_dens",
              fill.scale = tm_scale_intervals(values = pal, style = "quantile"),
              fill.legend = tm_legend(title = "Densité de population", orientation = "landscape"),
              id = "name") +
  tm_layout(frame = TRUE, legend.outside = TRUE,
            legend.outside.position = "bottom",
            legend.outside.size = 0.2,
...
mapIncome <- tm_shape(World) +
  tm_polygons(fill = "income_grp",
              fill.scale = tm_scale(values = pal,
                                    labels = c("Élevé (OCDE)", "Élévé (Non-OCDE)", "Intermédiaire supérieur", "Intermédiaire inférieur", "Faible")),
              fill.legend = tm_legend(title = "Niveau de PRB", orientation = "landscape"),
              id = "name") +
  tm_layout(frame = TRUE, legend.outside = TRUE,
            legend.outside.position = "bottom",
            legend.outside.size = 0.2,
...
```{r viridipal, message = FALSE, fig.cap="Les palettes de la bibliothèque `viridis`.", echo=FALSE, fig.height=2, fig.width = 5,fig.show="hold",fig.align="center"}
# SOURCE: code adapted from Gimond: https://github.com/mgimond/Spatial/blob/main/04_symbolizing_features.qmd
...
```{r brewerpal,  fig.height=10, fig.width = 5, fig.cap="Les palettes de la bibliothèque `RColorBrewer`.", echo=FALSE, fig.show="hold",fig.align="center"}
library(RColorBrewer)
display.brewer.all()
```
...
c4a_palettes(type = "div")   # palettes divergentes
```

<br>

Pour obtenir les couleurs d'une palette particulière, on utilise la fonction `c4a(name, n)` où `name` est le nom de la palette et `n` le nombre de couleurs souhaité. Par exemple :
...
###### Fonction tm_polygons() {.unnumbered #fct_tmfill}
...
```{r, dev = "png", fig.height=6}
tm_shape(Q) + tm_polygons(fill = "NUM_REG", fill.legend = tm_legend(title = "Régions"))
```
...
La fonction `tm_title()` permet d'ajouter un titre à une carte. Elle s'utilise comme une couche additionnelle :

```{r, dev = "png", fig.height=4}
map_Q + tm_title("Carte du Québec")
```
<br>

L'argument `size` contrôle la taille du titre, et `position` sa position sur la carte avec `tm_pos_in("horizontal", "vertical")` :

```{r, dev = "png", fig.height=4}
map_Q + tm_title("Carte du Québec", size = 0.8, position = tm_pos_in("right", "top"))
```
<br>
...
```{r, dev = "png", fig.height=6, fig.width=10}
Q1 <- map_Q + tm_grid()

Q2 <- map_Q + tm_graticules(crs = 4269)

tmap_arrange(Q1,Q2)
```
<br> 

Plusieurs options d'arguments existent pour les fonctions `tm_grid()` et `tm_graticules()`. Par exemple, il est possible de préciser le nombre de divisions sur l'axe des *x* (`n.x`) et sur l'axe des *y* (`n.y`)^[Noter que le nombre de divisions créé par `tmap` est approximativement celui demandé car `tmap` crée des divisions uniformément espacées situées sur des coordonnées de valeurs entières.], l'épaisseur du trait (`lwd`), la couleur (`col`) ou encore la taille de l'écriture (`labels.size`). Dans l'exemple ci-dessous, `tm_graticules()` est appelé sans préciser `crs`, ce qui utilise le CRS par défaut (EPSG:4326, WGS84) plutôt que NAD83 (EPSG:4269) — notez que la différence entre ces deux CRS est peu perceptible dans cette carte du Québec.
...
```{r eval = TRUE, warning = FALSE, echo = FALSE, dev = "png", fig.width=12, fig.height=4}
...
tmap_arrange(Qframe, Qbg, Qmargin, ncol=3, nrow=1, outer.margins = NULL)
```
<br>
...
* L'affichage de la légende (`show`). Par défaut la légende est affichée.
* La position de la légende, avec `position = tm_pos_in("horizontal", "vertical")` pour une légende à l'intérieur du cadre, ou `position = tm_pos_out("horizontal", "vertical")` pour une légende à l'extérieur du cadre. Par défaut, la légende est placée dans le coin où il y a le plus d'espace.
* L'orientation de la légende (`orientation`), soit `"portrait"` (par défaut) ou `"landscape"`.
* L'affichage d'un cadre autour de la légende (`frame`). Par défaut un cadre est affiché.
* La couleur du fond de la légende (`bg.couleur`). Par défaut le font est blanc.
* La taille du titre (`title.size`) et du texte (`text.size`).
* La hauteur (`item.height`) et la largeur (`item.width`) des éléments dans la légende.
...
```{r eval = TRUE, warning = FALSE, echo = FALSE, dev = "png", fig.width=10}
...
tmap_arrange(Q1, Q2, Q3, ncol = 3, nrow = 1)
```
...
* L'argument `color.saturation` définit le niveau de saturation des couleurs. La valeur par défaut est 1, et la valeur 0 donne une représentation en noir et blanc. Il est possible de donner des valeurs supérieures à 1 pour des couleurs très saturées.
* L'argument `color.sepia_intensity` est un nombre entre 0 et 1 qui définit le niveau de "chaleur" des couleurs. Plus sa valeur est grande, plus les couleurs ont une teinte jaune voire brune. La valeur par défaut est 0.
...
Q2 <- tm_shape(Q) + tm_polygons(fill = "NUM_REG",
                                fill.scale = tm_scale(values = "area9"),
                                 fill.legend = tm_legend(show = FALSE)) +
                    tm_layout(color.sepia_intensity = 0.6)

tmap_arrange(Q1, Q2, ncol = 2, nrow = 1)
```
...
Q3 <- tm_shape(Q) + tm_polygons(fill = "NUM_REG",
                                  fill.legend = tm_legend(show = FALSE)) +
                      tm_style("cobalt")
Q4 <- tm_shape(Q) + tm_polygons(fill = "NUM_REG",
                                  fill.legend = tm_legend(show = FALSE)) +
                      tm_style("gray")
...
Vous remarquerez que le style `gray` utilise des teintes de gris, ce qui le rend accessible aux personnes daltoniennes. Pour des cartes thématiques accessibles aux daltoniens, il est également recommandé d'utiliser des palettes de couleur adaptées, disponibles dans le package [`cols4all`](https://cols4all.github.io/cols4all-R/) (par exemple `fill.scale = tm_scale(values = "cols4all::yl_or_rd")`).
...
<!-- 
```{r, warning = FALSE, dev = "png", fig.height=6 }
tm_shape(Q) + tm_polygons(fill = "black", col = "white") +
                tm_layout(bg.color = "black",
                          frame = FALSE) +
                tm_labels(text = "NUM_REG", 
                          col = "NOM_REG",
                          col.scale = tm_scale(values = "brewer.paired"),
                          col.legend = tm_legend(title = "Régions administratives",
                                                 position = tm_pos_out("right", "center"),
                                                 text.color = "white",
                                                 title.color = "white",
                                                 bg.color = "black",
...
tm_shape(Q) +
  ...
  tm_lines(col = "ClsRte",
           ...
           col.legend = tm_legend(title = "Types de route"))
```
<br>
...
tm_shape(V) + tm_dots(fill = "darkblue", size = 1)
...
```{r, echo = FALSE, fig.cap = "Formes disponibles dans R et leur numéro (`shape`).", fig.height=2, fig.width=7, fig.align="center"}
par(mar = c(0, 0, 0, 0))
plot(0, type = "n", xlim = c(0, 26), ylim = c(1.1, 2.3), axes = FALSE, xlab = "", ylab = "")
for (i in 0:25) {
  points(i + 0.5, 2, pch = i, cex = 1.8, bg = "steelblue", col = "steelblue4")
  text(i + 0.5, 1.4, labels = i, cex = 0.8)
}
```
...
```{r, eval = TRUE, dev = "png", fig.height=6}
tm_shape(Q) + tm_fill() +
tm_shape(V) + tm_symbols(shape = 25,
                          fill = "darkblue",
                          size = 0.8,
                          fill.legend = tm_legend(show = FALSE)) +
              tm_labels(text = "Mncplts", size = 0.7)
```
...
E <- rast("Module6/Module6_donnees/Elevation/QC_Elevation.tif")
```
...
```{r, eval = TRUE, echo = TRUE,  dev = "png", fig.height=6}
tm_shape(E) + 
...
- `"fixed"` : crée des classes selon un choix explicite de limites, définies dans un vecteur assigné à l'argument `breaks`.
- `"equal"` : divise l'étendue des valeurs en `n` classes de largeur égale.
- `"pretty"` : choisit des limites de classes arrondies et visuellement agréables. C'est la méthode par défaut.
- `"quantile"` : divise les valeurs en quantiles, de sorte que chaque classe contient le même nombre de cellules.
- `"jenks"` : utilise l'algorithme de Jenks pour minimiser la variance à l'intérieur des classes.
...
```{r, eval = TRUE, echo = TRUE, dev = "png", fig.height=6, fig.width = 10 }
Q1 <- tm_shape(E) +
...
```{r, dev = "png", warning = FALSE, fig.height=5 }
# style fixed
Efixed <- tm_shape(E) +
...
tmap_arrange(carte_carre, carte_triangle)
```
...
tm_shape(Q) +
  ...
  tm_components(group_id = "légende", stack = "vertical")
...
```{r}
Q$Pop_prop_enfant <- round(100*Q$Pop_0_14/Q$Pop_tot)
```
<br>
...
- Sans **`fill.scale`** : tmap applique une échelle par défaut (`tm_scale()`) avec un style `"pretty"` et une palette automatique.
- **`tm_scale_continuous()`** : produit un dégradé de couleur continu, sans découpage en classes.
- **`tm_scale_intervals()`** : découpe les valeurs en classes discrètes selon un style de classification (`style = "fixed"`, `"quantile"`, `"equal"`, etc.). Lorsque `style = "fixed"`, les coupures sont définies manuellement avec l'argument `breaks`.
...
```{r, echo = FALSE, eval = TRUE, warning = FALSE, dev = "png", fig.height = 12, fig.width = 10}

# Par defaut, 
Qdefaut <- tm_shape(Q) + tm_polygons(fill = "Pop_prop_enfant",
                                     fill.legend = tm_legend(title = "Échelle par défaut", 
                                                             position = tm_pos_out("right", "center")))
                                     
# Style continu
Qcont<- tm_shape(Q) + tm_polygons(fill = "Pop_prop_enfant",
                                    fill.scale = tm_scale_continuous(),
                                    fill.legend = tm_legend( title = "Échelle continue",
                                                             position = tm_pos_out("right", "center")))
...
Qfixed <- tm_shape(Q) + tm_polygons(fill = "Pop_prop_enfant",
                                    fill.scale = tm_scale_intervals(
                                       style = "fixed",
                                       breaks = c(5, 10, 15, 20, 25, 30)),
                                    fill.legend = tm_legend( title = "Échelle prédéfinie",
                                                             position = tm_pos_out("right", "center")))
...
Qquant <- tm_shape(Q) + tm_polygons(fill = "Pop_prop_enfant",
                                    fill.scale = tm_scale_intervals(style = "quantile"),
                                    fill.legend = tm_legend( title = "Échelle quantile",
                                                             position = tm_pos_out("right", "center")))


tmap_arrange(Qdefaut,Qcont,Qfixed,Qquant)
...
```{r, eval=FALSE, warning = FALSE, dev = "png"}
...
E_Outaouais <- mask(E_Outaouais_ext, Q_Outaouais)
```
...
```{r, warning = FALSE, message = FALSE, dev = "png", fig.height = 6}
library(grid)
carte_princ
...
### Cartes interactives {#carto_interactive}

Jusqu'ici, toutes les cartes produites étaient statiques. La bibliothèque `tmap` permet également de produire des cartes interactives basées sur la bibliothèque [`leaflet`](https://leafletjs.com/), offrant des fonctionnalités de zoom, de déplacement et d'exploration des données.
...
ttm()              # basculer entre les deux modes
```

<br>

En mode interactif, le code de carte habituel produit une carte Leaflet. Par exemple, reprenons la carte des régions administratives du Québec :
...
L'argument `id` de la fonction `tm_polygons()` (ou de toute autre fonction de couche) définit le texte qui s'affiche au survol de la souris sur un élément de la carte. Ici, le nom de la région administrative apparaît au survol.
...
L'argument `hover` de `tm_symbols()` définit le texte qui s'affiche au survol de la souris sur un symbole. En lui passant le nom d'un attribut, seule la valeur de cet attribut est affichée — ici, le nom de la municipalité.
...
```{r, eval = FALSE, echo = TRUE}
tm_shape(E) +
  tm_raster(col.scale = tm_scale_intervals(style = "quantile", values = pal.elevation(5)))
```
...
N'oublions pas de revenir au mode statique à la fin :
